...
**Confirmed 2026-08-07 by Imran Qureshi: this is delegated access, not patient matching** — see
...
discussion (Jira `SD-1416`; Slack DM with Imran Qureshi and Alvin Henrick, 2026-07-17) before
...
base CMS v3.3.0 spec, all authored by Imran Qureshi (2026-07-15 through 2026-07-31 — i.e.,
...
2026-07-15 — two days *before* Imran told Sean in the DM "it's not a patient matching problem,
...
- It's Imran's own proposal, still under open review (a comment thread as of 2026-07-31).
...
**⚠️ SUPERSEDED on this specific point — see the 2026-08-07 update immediately below.** Imran
...
## 2026-08-07 update — Imran confirmed: this is delegated access, not patient matching; Rule 39 does not apply

Asked Imran directly (same Slack thread) whether Rule 39 — once its math is fixed — is the
...
On the consent-authority question, Imran also confirmed there's no existing answer anywhere,
...
**The actual technical shape (per Imran Qureshi, Slack DM, 2026-07-17): "it's not a patient
...
**The unresolved hard part (Imran, same thread):** *"Drawing the link between parent and child
...
- `RelatedPerson` FHIR resources derived from payer eligibility/enrollment feeds (subscriber →
  ...
  source, not a verified one. `NEEDS HUMAN DECISION — Sean/Imran`: confirm whether this data
...
none confirmed:
- `patient-matching` (this repo) — **ruled out as of 2026-08-07**: Imran confirmed this is
  delegated access, not patient matching, categorically, not just "unlikely to be the long-term
  home."
- `helix.personmatching-service` — **not ruled out**, corrected 2026-08-07: Imran's rejection
...
- A new or existing identity/consent service (unnamed at authoring time) — most plausible owner
  ...
  record, separate from matching, and per Imran likely needs to encode per-state legal
...
This doc does not resolve this. It stays here as a placeholder until Sean/Imran (and now,
...
- Treating a payer subscriber/household relationship as sufficient proof of legal guardianship
  by default — Imran explicitly flagged this as unreliable; any use of it needs an explicit,
  recorded risk acceptance from whoever owns that policy call.
- Building this in `patient-matching` if Sean/Imran decide it belongs in a different repo (see
...
- **ANSWERED 2026-08-07 by Imran Qureshi:** Does Imran see Proposal v3.3.3's Rule 39 as the
...
- **ANSWERED 2026-08-07 (by implication of the above), previously `NEEDS HUMAN DECISION —
  Sean/Imran`**: Does this get a Thread and scope anchor in `patient-matching` at all? **No** —
  confirmed out of scope for this repo, and for `helix.personmatching`/
  `helix.personmatching-service` too, per Imran's category rejection above. Whatever tracks this
  going forward needs a different home entirely (see the "which repo/service" item below, now
  the operative open question).
- **`NEEDS HUMAN DECISION — Sean/Imran/Legal-Compliance`** (sharpened 2026-08-07): what counts
  as sufficient proof that an account holder is authorized to access another person's (e.g., a
  minor's) clinical records? Imran confirmed directly: no existing answer, anywhere — "this is
  ...
  rule. Payer-eligibility subscriber/dependent data alone is explicitly *not* sufficient (Imran,
...
- **`NEEDS HUMAN DECISION — Sean`**: which repo/service/team owns this if it proceeds (see
  ...
  repo at all, per Imran's 2026-08-07 answer. More likely an access-control/consent capability
