...
> `ESCALATE`/`AMBIGUOUS` response) merged via PR #14 on 2026-07-31, so it's no longer a valid
> pick. Of the remaining pending sessions, 4 and 5 are both genuinely startable (4's hard code
> dependency on session 3 is satisfied; 5 has no dependencies at all); 6 is not yet startable
> (depends on 5). Picking 4: it's the next Tier-2 statistical-rigor milestone per
> `conventions.md` and doesn't share Line B's CMS-v3.3-spec-fetch dependency that 5/6 both
> carry. Note: session 4 has a `NEEDS HUMAN DECISION` (exact Databricks/Mongo table names) that
> must be resolved with Sean before any code is written.
