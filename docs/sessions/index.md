...
> flagging). **Session 16 is the new Suggested Next Session** — it's the smallest, most
> mechanical of the four, and sessions 17/18 both prefer (don't require) landing after it to
> avoid `table2_rules.py`/ID-reference churn. This work is independent of session 8's Tier-3
> harness below; both can proceed in parallel once session 8's `NEEDS HUMAN DECISION` items
> (in the moved repo) are answered.
>
...
|---|---------|--------|-----------|------|--------|-------------------|
| 16 | [session_16](pending/session_16.md) | Line B: CMS v3.4.0 migration | 6 (hard, satisfied — merged to `main` via `b7f44bc`) | M | pending | Renumber Category 1 rules to v3.4.0's clean 01-30 sequence; extend DOB +/-1 day fuzzy to (new) rules 01/02/03/10. |
| 17 | [session_17](pending/session_17.md) | Line B: CMS v3.4.0 migration | 16 (soft, file-conflict avoidance only) | L | pending | Relationship Linkage field (Table 3) + Rules 39/40 (guardian-verified minor / newborn-via-maternal-linkage) — built best-effort with the spec's own unresolved caveats documented. |
