...
> gate to satisfy (not a rule change), and no code dependency on session 3 — safe to start on
> a fresh branch from `main` right now, while session 3's PR is out for review. Session 5 is
> also independent and could be coded in parallel, but it's larger; session 1 is the fastest
> next win.
...
| 4 | [session_4](pending/session_4.md) | Evaluation & Statistical Rigor | 3 (in_review) | M | pending | Real-world FHIR data source for `rule_eval.py`, via reproducible queries |
...
Session 4 has a **hard code dependency** on session 3 (needs `evaluate_pair()`/`onc_loader.py`,
which only exist on session 3's unmerged branch) — don't start it until session 3 is in
`completed/`, not just `in_review/`. Sessions 5 and 6 have only a *merge-gate* dependency on
session 3 (see `conventions.md`'s statistical rigor gate): they can be coded in parallel with
session 3 sitting in `in_review/`, they just can't move to `completed/` before session 3 does.

## In Review

| # | Session | Thread | PR | One-line summary |
|---|---------|--------|----|-------------------|
| 3 | [session_3](in_review/session_3.md) | Evaluation & Statistical Rigor | [#11](https://github.com/icanbwell/patient-matching/pull/11) | ONC self-match baseline wired to `rule_eval.py` (1M-record dataset, not the ~28K assumed); pairwise matcher + sampled negatives. Work done, checks green, awaiting Sean's merge go-ahead. |
