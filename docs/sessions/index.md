...
| 13 | [session_13](pending/session_13.md) | Phase 2: production candidate-retrieval scaling | None | S | pending — design captured; package name decided (`cms-hte-patient-matching`); mechanism switched to token-based twine per `helix.fhir.client.sdk`'s pattern (Imran's steer); blocked on Imran minting a PyPI API token (Open Question 2 — well-evidenced as his own action) before a feature branch can start | Publish `cms-hte-patient-matching` to PyPI via token-based twine (matching `helix.fhir.client.sdk`/`helix.personmatching`, not OIDC Trusted Publishing); fixes needed: missing `[build-system]` in `pyproject.toml`, stale `setup.py`, a stray `patientmatching/` dir, rewritten `python-publish.yml`, and missing Makefile publish targets |
