...
def _make_token(iss: str) -> str:
    return jwt.encode({"iss": iss, "sub": "u1"}, key="unused-test-signing-key-32-bytes-min", algorithm="HS256")


def _fake_discovered_verifier(
    jwks_uri: str, claims: Dict[str, Any]
) -> TokenVerifier:
...
class TestMultiIssuerTokenVerifier:
    async def test_accepts_whitelisted_issuer(self) -> None:
        ...
        with patch.object(
            TokenVerifier, "from_oidc_discovery", AsyncMock(return_value=fake_verifier)
        ):
    ...
    async def test_rejects_non_whitelisted_issuer(self) -> None:
    ...
    async def test_caches_verifier_per_issuer(self) -> None:
        token = _make_token("https://good.example.com")
        fake_verifier = _fake_discovered_verifier(
    ...
    async def test_missing_iss_claim_rejected(self) -> None:
        token = jwt.encode({"sub": "u1"}, key="unused-test-signing-key-32-bytes-min", algorithm="HS256")
