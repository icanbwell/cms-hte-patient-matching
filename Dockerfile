...
WORKDIR /usr/src/patientmatching/
...
COPY Pipfile* /usr/src/patientmatching/
...
ENV PROJECT_DIR=/usr/src/patientmatching
ENV FLASK_APP=patientmatching.api
...
COPY ./patientmatching ${PROJECT_DIR}

# Copy the Pipfile.lock from the previous stage in case it was locked
COPY --from=python_packages /usr/src/patientmatching/Pipfile.lock /usr/src/patientmatching/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/patientmatching/Pipfile.lock /tmp/Pipfile.lock
